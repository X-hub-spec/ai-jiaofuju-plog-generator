...
把 Markdown、Word/docx、文章内容和配图，自动整理成适合小红书发布的 3:4 图文 PLOG。
...
- 把带配图的 Word/docx 内容整理成 PLOG。
- 把你直接发来的文字和图片素材整理成 PLOG。
- 自动生成第一页封面，后续页面排正文、图片、重点高亮和代码块。
- 支持两种固定风格：`书面阅读感`、`专业黑金`。
...
把这个仓库里的 skill 复制到 Codex skills 目录：
...
Codex 会根据这些内容自动分页、排版、挑选适合高亮的金句，并输出可预览的 HTML 和可发布的 PNG 图片。
...
- `index.html`：本地预览页面。
- `exports/page-01.png`、`page-02.png` 等：小红书发布用图片。
- `assets/`：复制后的图片素材。
