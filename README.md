...
- Markdown：`examples/demo/claude-fable-5/article.md`
- 主题图：`examples/demo/claude-fable-5/cover.png`
- 书面阅读感配置：`examples/demo/claude-fable-5/config-reading.json`
- 专业黑金配置：`examples/demo/claude-fable-5/config-pro.json`

运行书面阅读感 Demo：

```bash
python3 xhs-markdown-plog/scripts/build_plog.py examples/demo/claude-fable-5/config-reading.json output/demo-reading
```

运行专业黑金 Demo：

```bash
python3 xhs-markdown-plog/scripts/build_plog.py examples/demo/claude-fable-5/config-pro.json output/demo-pro
```
...
node xhs-markdown-plog/scripts/export_pages.mjs output/demo-reading/index.html output/demo-reading/exports
```
...
- 外部 AI 或其他软件调用本 skill 时，必须把原文逐字传入 `content` 或 `content_file`，不能先让模型做“整理/提炼/优化文案”的中间步骤。
...
Playwright 不是生成 PLOG 的必需依赖：如果你只需要 `index.html` 预览，Python 就够了。Playwright 只用于仓库自带的自动截图导出脚本，把 HTML 页面批量导出为 `1080 × 1440` PNG。
...
```json
{
  ...
  "content_file": "/absolute/path/article.md",
  "assets": [
